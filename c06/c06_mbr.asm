  ...
  start:
         ...
         ; 单纯的movsb/movsw只能执行1次, 需要加上rep(repeat) 才能执行CX次. 意思是CX不为0则重复
         ; movsb/movsw 每执行1次, CX的内容自动减一
  ...
  digit:
         ...
         div si
         ; 在8086处理器中, 只能使用BX SI DI BP来提供偏低地址. 可以在任何带有内存操作数的质量中使用BX DI SI来提供偏低地址
         mov [bx],dl                   ;保存数位
         inc bx                         ;bx做为内存地址索引
         ...
         mov si,4                       ;个数保存在si中
   show:
        ; 8086处理器中, 只允许 bx/bp + si/di的组合使用来代表内存操作数
